...
Dokument: ${fileName}`
  ...
  try {
    ...
    const events = Array.isArray(parsed.events) ? parsed.events.map((item, index) => { const rawLabel = String(item.label || '').slice(0, 120); const gender = ['Dam', 'Herr', 'D', 'H', 'Alla'].includes(item.gender) && item.gender !== 'Alla' ? item.gender : (/(^|[\s/·])H([\s/·]|$)/i.test(rawLabel) ? 'H' : /(^|[\s/·])D([\s/·]|$)/i.test(rawLabel) ? 'D' : 'Alla'); const ageClass = String(item.ageClass || '').trim() || (rawLabel.match(/(^|[\s/·])([A-E])([\s/·]|$)/i)?.[2] || 'Alla åldrar'); return { eventOrder: Number.isInteger(item.eventOrder) ? item.eventOrder : index + 1, eventNumber: String(item.eventNumber || '').slice(0, 20), itemType: ['race', 'pause', 'award', 'info'].includes(item.itemType) ? item.itemType : 'race', entryAllowed: item.entryAllowed !== false && item.itemType !== 'pause' && item.itemType !== 'award' && item.itemType !== 'info', gender, ageClass: ageClass.slice(0, 60), distanceMeters: Number.isInteger(item.distanceMeters) ? item.distanceMeters : null, stroke: String(item.stroke || 'Annat').slice(0, 30), label: cleanCompetitionLabel(rawLabel, gender, ageClass) } }).filter((item) => item.label).slice(0, 300) : []
