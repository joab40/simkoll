...
async function createAiInsight(request, response) {
  ...
  const prompt = `Du är ett försiktigt analysstöd för simtränare. Analysera endast datan nedan. Skriv på svenska, konkret och uppmuntrande. Dra inga medicinska slutsatser och hitta inte på orsaker. Om underlaget är litet, säg det tydligt. Jämför bara liknande pass och föregående period när värden finns. Använd volym i meter, antal pass, tidsåtgång, passinriktning (t.ex. fart, tröskel, syra, distans, teknik och återhämtning), temperatur, RPE, fartkänsla och spridning för att beskriva mönster — inte för att fastställa orsaker. Kommentera 14 000 meter som en observerad mängd i perioden, inte som bra eller dålig utan att känna till tränarens plan. Relatera vid behov mängd och upplevd ansträngning till varandra. VIKTIGT: om current.sickDays eller current.restDays är större än 0 ska du alltid nämna det neutralt under attention/följ upp. Skriv exempelvis att statusen är registrerad och att tränaren kan följa upp med simmaren; antyd aldrig diagnos eller orsak. Returnera ENDAST giltig JSON med exakt dessa nycklar: summary (max 280 tecken), positives (array med max 3 korta strängar), attention (array med max 3 korta strängar), limitations (array med max 2 korta strängar). Data: ${JSON.stringify(safe)}`
  try {
