...
function SwimmerPlanning({ code, onBack }) {
  ...
  const visiblePlans = plans.filter((plan) => plan.date >= startKey && plan.date < endKey).sort((a, b) => a.date.localeCompare(b.date))
  ...
  return <section className="swimmer-planning"><button className="back-button inline" onClick={onBack}>← Tillbaka</button><div className="period-heading"><div><p className="eyebrow">Planering</p><h1>{weekOffset === 0 ? 'Den här veckan' : weekOffset === -1 ? 'Förra veckan' : weekOffset === 1 ? 'Nästa vecka' : 'Veckoplanering'}</h1><small>Planerade aktiviteter för din grupp.</small></div></div><div className="swimmer-planning-week-controls"><button type="button" className="secondary-button" onClick={() => setWeekOffset((value) => value - 1)}>← Förra veckan</button><strong>{weekLabel}</strong><button type="button" className="secondary-button" onClick={() => setWeekOffset((value) => value + 1)}>Nästa vecka →</button></div>{visiblePlans.length ? <div className="swimmer-planning-list">{visiblePlans.map((plan) => <article key={plan.id}><p className="eyebrow">{plan.date}</p><h2>{plan.title}</h2><p>{plan.activityType === 'swim' ? '🏊 Simning' : plan.activityType === 'strength' ? '🏋️ Styrka' : plan.activityType === 'dryland' ? '🤸 Landträning' : '🏆 Tävling'}{plan.focus ? ` · ${plan.focus}` : ''}</p><small>{[plan.distanceMeters && `${plan.distanceMeters} m`, plan.durationMinutes && `${plan.durationMinutes} min`, plan.location].filter(Boolean).join(' · ')}</small></article>)}</div> : <p className="empty">Ingen planering publicerad för den här veckan.</p>}</section>
}
...
function Aljakten({ code, onBack, preview = false }) {
  ...
  useEffect(() => {
    ...
    const draw = () => { const game = gameRef.current; const now = game.time || 0; const gradient = ctx.createLinearGradient(0, 0, width, height); gradient.addColorStop(0, '#101d3e'); gradient.addColorStop(1, '#173f58'); ctx.fillStyle = gradient; ctx.fillRect(0, 0, width, height); ctx.fillStyle = 'rgba(115,225,226,.22)'; for (let i = 0; i < 30; i += 1) { const x = (i * 67 + now * (7 + i % 4)) % width; const y = height - ((now * (11 + i % 5) + i * 83) % (height + 40)); const radius = 1.5 + (i % 3); ctx.globalAlpha = .28 + ((Math.sin(now * 2 + i) + 1) / 2) * .5; ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill() } ctx.globalAlpha = 1; (game.hazards || []).forEach((hazard, index) => { const x = ((hazard.x + now * hazard.speed) % (width + 100)) - 50; const y = hazard.y + Math.sin(now * .8 + index * 2) * 12; ctx.globalAlpha = .92; ctx.font = `${hazard.size}px "Apple Color Emoji", "Segoe UI Emoji", sans-serif`; ctx.textAlign = 'center'; ctx.fillText(hazard.emoji, x, y); ctx.globalAlpha = 1 }); if (game.star) { const pulse = 1 + Math.sin(now * 8) * .14; const outer = 14 * pulse; const inner = 6 * pulse; ctx.save(); ctx.translate(game.star.x, game.star.y); ctx.rotate(now * .8); ctx.globalAlpha = .55 + ((Math.sin(now * 8) + 1) / 2) * .45; ctx.fillStyle = '#ffe27a'; ctx.shadowColor = '#ffe27a'; ctx.shadowBlur = 18; ctx.beginPath(); for (let i = 0; i < 10; i += 1) { const radius = i % 2 ? inner : outer; const angle = -Math.PI / 2 + i * Math.PI / 5; ctx.lineTo(Math.cos(angle) * radius, Math.sin(angle) * radius) } ctx.closePath(); ctx.fill(); ctx.restore(); ctx.globalAlpha = 1 } (game.foods || []).forEach((food, index) => { const pulse = Math.sin(now * 3.5 + index) * 1.4; ctx.fillStyle = food.color; ctx.shadowColor = food.color; ctx.shadowBlur = 10; ctx.beginPath(); ctx.arc(food.x, food.y, food.radius + pulse, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0; ctx.fillStyle = 'rgba(255,255,255,.7)'; ctx.beginPath(); ctx.arc(food.x - food.radius * .28, food.y - food.radius * .28, Math.max(1.5, food.radius * .18), 0, Math.PI * 2); ctx.fill() }); const snake = game.snake || []; const growth = .72 + Math.min(.65, (game.score || 0) * .035); const bodyRadius = 6 + growth * 3.2; snake.forEach((part, index) => { ctx.fillStyle = index === 0 ? '#d9f56a' : `hsl(${156 + index * 2} 55% ${53 - Math.min(18, index)}%)`; ctx.beginPath(); ctx.arc(part.x, part.y, index === 0 ? bodyRadius + 1.5 : bodyRadius, 0, Math.PI * 2); ctx.fill(); if (index === 0) { const heading = game.heading || 0; const frontX = Math.cos(heading) * bodyRadius * .42; const frontY = Math.sin(heading) * bodyRadius * .42; const sideX = -Math.sin(heading) * bodyRadius * .42; const sideY = Math.cos(heading) * bodyRadius * .42; ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(part.x + frontX + sideX, part.y + frontY + sideY, 3.5, 0, Math.PI * 2); ctx.arc(part.x + frontX - sideX, part.y + frontY - sideY, 3.5, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#173047'; ctx.beginPath(); ctx.arc(part.x + frontX + sideX, part.y + frontY + sideY, 1.7, 0, Math.PI * 2); ctx.arc(part.x + frontX - sideX, part.y + frontY - sideY, 1.7, 0, Math.PI * 2); ctx.fill() } }); ctx.fillStyle = 'rgba(255,255,255,.92)'; ctx.font = '800 24px Manrope, sans-serif'; ctx.textAlign = 'center'; ctx.fillText(String(game.score || 0), width / 2, 34) }
    const loop = (time) => { const game = gameRef.current; if (!game.running) { draw(); return } const delta = Math.min(.035, (time - game.last) / 1000); game.last = time; game.time = time / 1000; const targetDistance = Math.hypot(game.target.x - game.snake[0].x, game.target.y - game.snake[0].y); if (targetDistance > 10) game.heading = Math.atan2(game.target.y - game.snake[0].y, game.target.x - game.snake[0].x); const speed = 78; const move = speed * delta; const head = { x: game.snake[0].x + Math.cos(game.heading) * move, y: game.snake[0].y + Math.sin(game.heading) * move }; const canHitSelf = game.snake.length > 24; const hitSelf = canHitSelf && game.snake.slice(20).some((part) => Math.hypot(head.x - part.x, head.y - part.y) < 9); const hitHazard = (game.hazards || []).some((hazard) => { const x = ((hazard.x + game.time * hazard.speed) % (width + 100)) - 50; const y = hazard.y + Math.sin(game.time * .8 + hazard.phase) * 12; return x > -35 && x < width + 35 && Math.hypot(head.x - x, head.y - y) < hazard.radius + 9 }); if (head.x < 13 || head.x > width - 13 || head.y < 45 || head.y > height - 13 || hitSelf || hitHazard) { game.running = false; setStatus('over'); if (!preview) apiRequest('/api/points', code, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'submit-game-score', gameKey: 'aljakten', score: game.score }) }).then(setGameData).catch(() => {}); draw(); return } game.snake.unshift(head); while (game.snake.length > 9 + game.score * 5) game.snake.pop(); const eatenIndex = (game.foods || []).findIndex((food) => Math.hypot(head.x - food.x, head.y - food.y) < food.radius + 11); if (eatenIndex >= 0) { const eaten = game.foods[eatenIndex]; game.score += eaten.points; setScore(game.score); game.foods[eatenIndex] = { x: 22 + Math.random() * (width - 44), y: 60 + Math.random() * (height - 82), radius: eaten.radius, points: eaten.points, color: eaten.color } } draw(); game.frame = requestAnimationFrame(loop) }
    gameRef.current.startLoop = () => { gameRef.current.last = performance.now(); gameRef.current.frame = requestAnimationFrame(loop) }
    const moveTarget = (event) => { const rect = canvas.getBoundingClientRect(); const game = gameRef.current; const head = game.snake?.[0] || { x: width / 2, y: height / 2 }; const pointer = { x: (event.clientX - rect.left) * width / rect.width, y: (event.clientY - rect.top) * height / rect.height }; const desired = Math.atan2(pointer.y - head.y, pointer.x - head.x); const current = game.heading || 0; let difference = desired - current; while (difference > Math.PI) difference -= Math.PI * 2; while (difference < -Math.PI) difference += Math.PI * 2; const limited = current + Math.max(-.055, Math.min(.055, difference)); game.target = { x: head.x + Math.cos(limited) * 150, y: head.y + Math.sin(limited) * 150 } }
    const keydown = (event) => { const game = gameRef.current; const step = 58; if (event.key === 'ArrowLeft') game.target.x -= step; if (event.key === 'ArrowRight') game.target.x += step; if (event.key === 'ArrowUp') game.target.y -= step; if (event.key === 'ArrowDown') game.target.y += step; game.target.x = Math.max(15, Math.min(width - 15, game.target.x)); game.target.y = Math.max(48, Math.min(height - 15, game.target.y)) }
  ...
  useEffect(() => { const onFullscreenChange = () => setFullscreen(document.fullscreenElement === boardRef.current); document.addEventListener('fullscreenchange', onFullscreenChange); return () => document.removeEventListener('fullscreenchange', onFullscreenChange) }, [])
  const toggleFullscreen = async () => { try { if (document.fullscreenElement) await document.exitFullscreen(); else await boardRef.current?.requestFullscreen?.() } catch { setFullscreen(false) } }
  const start = () => { const center = { x: 70, y: 240 }; const foodColors = ['#ffd166', '#f6bd60', '#f7aef8', '#8bd9e2', '#c9f05a']; const foods = Array.from({ length: 7 }, (_, index) => ({ x: 40 + Math.random() * 280, y: 70 + Math.random() * 355, radius: index % 3 === 0 ? 10 : index % 2 === 0 ? 8 : 6, points: index % 3 === 0 ? 2 : 1, color: foodColors[index % foodColors.length] })); const hazards = [{ x: -120, y: 135, speed: -30, size: 22, radius: 13, emoji: '🐟', color: '#63e6ff', phase: 0 }, { x: 390, y: 320, speed: -20, size: 30, radius: 19, emoji: '🦈', color: '#c5a7ff', phase: 2.5 }, { x: -260, y: 410, speed: -26, size: 20, radius: 12, emoji: '🐟', color: '#8df5c1', phase: 4 }]; gameRef.current = { ...gameRef.current, running: true, heading: 0, snake: [center, { x: 56, y: 240 }, { x: 42, y: 240 }], hazards, target: { x: 260, y: 240 }, foods, score: 0, time: 0, last: performance.now(), frame: null }; setScore(0); setStatus('running'); window.setTimeout(() => gameRef.current.startLoop?.(), 0) }
  return <section className="game-page aljakten-page"><button className="back-button inline" onClick={onBack}>← Tillbaka</button><div className="game-layout"><div><p className="eyebrow">Veckans spel · Åljakten</p><h1>Åljakten 🐍</h1><p className="game-intro">Styr med musen, fingret eller piltangenterna. Samla energibubblor och väx utan att nudda kanten eller dig själv.</p><div className="aljakten-actions"><button className="secondary-button" onClick={toggleFullscreen}>{fullscreen ? '↙ Lämna fullskärm' : '⛶ Fullskärm'}</button></div><div ref={boardRef} className="game-board aljakten-board"><canvas ref={canvasRef} width="360" height="480" aria-label="Åljakten-spelet" />{status !== 'running' && <div className="game-overlay"><span>{status === 'over' ? '💥' : '🐍'}</span><strong>{status === 'over' ? `Du fick ${score} poäng` : 'Redo?'}</strong><small>{status === 'over' ? 'Försök växa ännu längre!' : 'Håll pekaren framför ålen och samla bubblorna.'}</small><button className="primary-button" onClick={start}>{status === 'over' ? 'Spela igen' : 'Starta spelet'}</button></div>}</div></div><section className="game-scoreboard"><p className="eyebrow">Åljakten · all time</p><h2>Topplistan</h2><p className="game-best">Ditt rekord: <strong>{gameData.ownBest || 0}</strong></p>{gameData.leaderboard?.length ? <div>{gameData.leaderboard.map((item) => <article key={item.profileId}><b>{item.rank}</b><span>{item.emoji}</span><strong>{item.displayName}</strong><em>{item.score}</em></article>)}</div> : <p className="empty">Ingen har spelat ännu.</p>}<small>Samla så många energibubblor du kan. Rekorden sparas över tid.</small></section></div></section>
}
...
function Community({ profile, code, points, customPepEnabled = true, onBack, onPointsChange }) {
