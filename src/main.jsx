...
const visibleProfiles = profiles.filter((profile) => profile.tempusId && (profileFilter === 'all' || profile.id === profileFilter))
  ...
  return <section className="competition-results"><div className="period-heading"><div><p className="eyebrow">Tempus Open</p><h1>Tävlingsresultat</h1></div><button className="primary-button" disabled={loading} onClick={onSync}>{loading ? 'Hämtar…' : 'Hämta Tempus-data för alla'}</button></div><div className="library-controls"><label>Simmare<select value={profileFilter} onChange={(event) => setProfileFilter(event.target.value)}><option value="all">Alla simmare</option>{profiles.filter((profile) => profile.tempusId).map((profile) => <option key={profile.id} value={profile.id}>{profile.emoji} {profile.displayName}</option>)}</select></label></div><div className="competition-sync-list">{profiles.filter((profile) => profile.tempusId).map((profile) => <div key={profile.id}><span>{profile.emoji} {profile.displayName}</span><small>Tempus-ID {profile.tempusId}</small><button className="secondary-button" disabled={loading} onClick={() => onSyncProfile(profile.id)}>Hämta</button></div>)}</div>{visibleProfiles.length ? <div className="competition-profile-list">{visibleProfiles.map((profile) => { const grouped = new Map(); results.filter((item) => item.profile_id === profile.id).forEach((item) => { const key = `${item.event}|${item.pool || ''}`; if (!grouped.has(key)) grouped.set(key, []); grouped.get(key).push(item) }); return <details className="competition-profile-card" key={profile.id}><summary><span>{profile.emoji} {profile.displayName}</span><small>{[...grouped.values()].length} event</small></summary>{grouped.size ? <div className="competition-event-list">{[...grouped.entries()].sort(([a], [b]) => { const aa = strokeOrder(a.split('|')[0]); const bb = strokeOrder(b.split('|')[0]); return aa[0] - bb[0] || aa[1] - bb[1] || aa[2].localeCompare(bb[2], 'sv') }).map(([key, items]) => { const ordered = items.slice().sort((a, b) => (a.result_time || 999999) - (b.result_time || 999999)); const best = ordered[0]; return <details key={key}><summary><span>{best.event}<small>{best.pool || 'Bassäng saknas'}</small></span><b>{best.swim_time}</b></summary><div className="competition-history"><strong>Historik senaste 3 åren</strong>{items.slice().sort((a, b) => b.result_date.localeCompare(a.result_date)).map((item) => <span key={item.id}>{new Date(item.result_date).toLocaleDateString('sv-SE')} · {item.swim_time}{item.aqua_points != null ? ` · ${item.aqua_points} Aqua` : ''}</span>)}</div></details> })}</div> : <p className="empty">Inga sparade resultat. Klicka Hämta.</p>}</details> })}</div> : <p className="empty">Inga sparade resultat ännu. Hämta Tempus-data för en simmare eller hela gruppen.</p>}</section>
}

function WorkoutTrendAnalysis({ analysis }) {
  if (!analysis?.focuses?.length && !analysis?.workload?.length) return null
  return <section className="workout-trend-analysis"><div className="workout-trend-heading"><div><p className="eyebrow">Passens innehåll och upplevelse</p><h2>Hur passen upplevs</h2></div><small>Inriktning jämförs med simmarnas svar</small></div>{analysis.focuses?.length > 0 && <div className="workout-focus-list">{analysis.focuses.map((item) => <article key={item.focus}><div><strong>{item.label}</strong><small>{item.workouts} pass{item.distance ? ` · ${item.distance.toLocaleString('sv-SE')} m` : ''}{item.duration ? ` · ${item.duration} min` : ''}</small></div><div className="workout-focus-metrics"><span>{item.rpe == null ? '–' : `RPE ${item.rpe}`}</span><span>{item.feeling == null ? '–' : `Känsla ${item.feeling}/5`}</span><span>{item.passRating == null ? '–' : `Pass ${item.passRating}/5`}</span></div></article>)}</div>}{analysis.workload?.length > 0 && <div className="workload-trend"><p className="eyebrow">Mängd över tid</p>{analysis.workload.map((item) => <div key={item.weekStart}><span>{new Date(`${item.weekStart}T12:00:00`).toLocaleDateString('sv-SE', { day: 'numeric', month: 'short' })}</span><b>{item.distance ? `${item.distance.toLocaleString('sv-SE')} m` : '–'}</b><small>{item.duration ? `${item.duration} min` : 'Ingen tidsdata'} · {item.workouts} pass</small></div>)}</div>}</section>
}
      ...
      {visibleProfiles.length ? <div className="swimmer-grid">{visibleProfiles.map((profile) => {
        ...
        return <article key={profile.id} className={`swimmer-card ${isExpanded ? 'expanded' : 'compact'}`}>
          ...
          {isExpanded && <div className="swimmer-card-details">
            ...
            <details className="tempus-edit"><summary>Tempus Open</summary><form onSubmit={(event) => { event.preventDefault(); const value = event.currentTarget.elements.tempusId.value.trim(); apiRequest('/api/profiles', code, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'set-tempus-id', profileId: profile.id, tempusId: value }) }).then(() => { setTempusResults((current) => ({ ...current, [profile.id]: undefined })); return onProfilesChange() }).catch((error) => window.alert(error.message)) }}><label>Tempus-ID<input name="tempusId" inputMode="numeric" pattern="[0-9]{1,12}" maxLength="12" defaultValue={profile.tempusId || ''} placeholder="t.ex. 273688" /></label><button type="submit">Spara ID</button></form>{profile.tempusId ? <><button type="button" disabled={tempusLoading[profile.id]} onClick={() => loadTempusResults(profile)}>{tempusLoading[profile.id] ? 'Hämtar…' : 'Hämta senaste resultat'}</button><a href={`https://www.tempusopen.se/swimmers/${profile.tempusId}/swimming`} target="_blank" rel="noreferrer">Öppna i Tempus Open ↗</a>{tempusResults[profile.id]?.results?.length > 0 && <div className="tempus-results"><strong>Bästa resultat från Tempus Open</strong>{tempusResults[profile.id].results.map((item) => <div key={`${item.event}-${item.pool}-${item.date}`}><span>{item.event}<small>{item.pool || 'Bassäng saknas'}</small></span><b>{item.time}</b><small>{item.aquaPoints != null ? `${item.aquaPoints} Aqua` : ''} · {new Date(item.date).toLocaleDateString('sv-SE')}</small></div>)}</div>}{tempusResults[profile.id]?.history?.length > 0 && <details className="tempus-history"><summary>Visa utveckling senaste 3 åren</summary>{tempusResults[profile.id].history.map((group) => <div key={`${group.event}-${group.pool}`}><strong>{group.event} · {group.pool || 'Bassäng saknas'}</strong>{group.items.map((item) => <span key={`${item.date}-${item.time}`}>{new Date(item.date).toLocaleDateString('sv-SE')} · {item.time}{item.aquaPoints != null ? ` · ${item.aquaPoints} Aqua` : ''}</span>)}</div>)}</details>}</> : <small>Lägg in ID för att länka till simmarens publika resultat.</small>}</details>
