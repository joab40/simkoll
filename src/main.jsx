...
function TrendLineChart({ items }) {
  ...
  const lineSegments = (metric) => items.reduce((segments, item, index) => { const value = Number(item[metric.key]); if (!Number.isFinite(value)) return segments; const last = segments[segments.length - 1]; if (last && last[last.length - 1].index === index - 1) last.push({ index, value }); else segments.push([{ index, value }]); return segments }, [])
  return <div className="trend-line-section"><div className="trend-line-wrap"><svg className="trend-line-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Träningsupplevelse per genomfört pass">{[0, 1, 2, 3, 4, 5].map((tick) => <g key={tick}><line x1={left} x2={width - right} y1={y(tick, 5)} y2={y(tick, 5)} /><text x={left - 8} y={y(tick, 5) + 3}>{tick}</text></g>)}{metrics.map((metric) => lineSegments(metric).map((segment, segmentIndex) => <polyline key={`${metric.key}-${segmentIndex}`} className={`trend-line-${metric.key}`} points={segment.map((point) => `${x(point.index)},${y(point.value, metric.max)}`).join(' ')} fill="none" stroke={metric.color} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />))}{metrics.map((metric) => items.map((item, index) => { const value = Number(item[metric.key]); return Number.isFinite(value) ? <circle key={`${metric.key}-${index}`} className={`trend-dot-${metric.key}`} cx={x(index)} cy={y(value, metric.max)} r="4" fill={metric.color} onMouseEnter={() => setHoverIndex(index)} onMouseLeave={() => setHoverIndex(null)} /> : null }))}{items.map((item, index) => <text className="trend-x-label" key={`${item.date}-${index}`} x={x(index)} y={height - 10}>{new Date(`${item.date}T12:00:00`).toLocaleDateString('sv-SE', { day: 'numeric', month: 'short' })}</text>)}</svg></div><div className="trend-line-legend">{metrics.map((metric) => <span key={metric.key}><i style={{ background: metric.color }} />{metric.label}</span>)}</div>{hoverIndex != null && <div className="trend-hover-card"><strong>{new Date(`${items[hoverIndex].date}T12:00:00`).toLocaleDateString('sv-SE', { day: 'numeric', month: 'short' })}</strong>{metrics.map((metric) => Number.isFinite(Number(items[hoverIndex][metric.key])) && <span key={metric.key}><i style={{ background: metric.color }} />{metric.label}: {metric.key === 'distanceMeters' ? `${Number(items[hoverIndex][metric.key]).toLocaleString('sv-SE')} m` : items[hoverIndex][metric.key]}</span>)}</div>}<div className="trend-pass-list">{items.map((item, index) => <article key={`${item.date}-${index}`}><strong>{new Date(`${item.date}T12:00:00`).toLocaleDateString('sv-SE', { day: 'numeric', month: 'short' })}</strong><span>{item.title}{item.focus ? ` · ${item.focus}` : ''}</span><small>{[item.distanceMeters && `${Number(item.distanceMeters).toLocaleString('sv-SE')} m`, item.durationMinutes && `${item.durationMinutes} min`, `${item.count} svar`].filter(Boolean).join(' · ')}</small></article>)}</div></div>
}
