      ...
      {visibleProfiles.length ? <div className="swimmer-grid">{visibleProfiles.map((profile) => {
        ...
        return <article key={profile.id} className={`swimmer-card ${isExpanded ? 'expanded' : 'compact'}`}>
          ...
          {isExpanded && <div className="swimmer-card-details">
            ...
            <div className="tempus-edit"><strong>Tempus-ID</strong><form onSubmit={(event) => { event.preventDefault(); const value = event.currentTarget.elements.tempusId.value.trim(); apiRequest('/api/profiles', code, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'set-tempus-id', profileId: profile.id, tempusId: value }) }).then(() => { setTempusResults((current) => ({ ...current, [profile.id]: undefined })); return onProfilesChange() }).catch((error) => window.alert(error.message)) }}><label><input name="tempusId" inputMode="numeric" pattern="[0-9]{1,12}" maxLength="12" defaultValue={profile.tempusId || ''} placeholder="t.ex. 273688" /></label><button type="submit">Spara ID</button></form>{profile.tempusId ? <><button type="button" disabled={tempusLoading[profile.id]} onClick={() => loadTempusResults(profile)}>{tempusLoading[profile.id] ? 'Hämtar…' : 'Hämta resultat'}</button><a href={`https://www.tempusopen.se/swimmers/${profile.tempusId}/swimming`} target="_blank" rel="noreferrer">Öppna Tempus ↗</a>{tempusResults[profile.id]?.results?.length > 0 && <div className="tempus-results"><strong>Bästa resultat från Tempus Open</strong>{tempusResults[profile.id].results.map((item) => <div key={`${item.event}-${item.pool}-${item.date}`}><span>{item.event}<small>{item.pool || 'Bassäng saknas'}</small></span><b>{item.time}</b><small>{item.aquaPoints != null ? `${item.aquaPoints} Aqua` : ''} · {new Date(item.date).toLocaleDateString('sv-SE')}</small></div>)}</div>}{tempusResults[profile.id]?.history?.length > 0 && <details className="tempus-history"><summary>Visa utveckling</summary>{tempusResults[profile.id].history.map((group) => <div key={`${group.event}-${group.pool}`}><strong>{group.event} · {group.pool || 'Bassäng saknas'}</strong>{group.items.map((item) => <span key={`${item.date}-${item.time}`}>{new Date(item.date).toLocaleDateString('sv-SE')} · {item.time}{item.aquaPoints != null ? ` · ${item.aquaPoints} Aqua` : ''}</span>)}</div>)}</details>}</> : <small>Spara ett ID för att länka till Tempus Open.</small>}</div>
