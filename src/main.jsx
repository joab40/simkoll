...
function Swimgames({ code, onBack }) {
  ...
  const resetOwnScore = async () => {
    if (!window.confirm('Nollställa din Swimgames-tid? Detta tar bort bara ditt eget resultat.')) return
    try {
      const next = await apiRequest('/api/points', code, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'reset-game-score', gameKey: 'swimgames' }) })
      setGameData(next)
    } catch (error) { window.alert(error.message) }
  }
