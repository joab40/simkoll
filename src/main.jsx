...
function GoalCompliance({ data, code, profile }) {
  ...
  return <section className="goal-compliance">
    ...
    {data.goalProgress && <GoalProgressBlock title="Avslutade veckor · simning" completed={data.goalProgress.completed} target={data.goalProgress.expected} percentage={data.goalProgress.percentage} detail={`Målet nåddes ${data.goalProgress.weeksReached} av ${data.goalProgress.weeksCount} veckor`} />}
    ...
    <form onSubmit={save}><p className="eyebrow">Styrka och landträning</p><label>Styrkepass / vecka<input type="number" min="0" max="7" value={targets.strengthTarget} onChange={(event) => setTargets({ ...targets, strengthTarget: event.target.value })} /></label><label>Landträningar / vecka<input type="number" min="0" max="7" value={targets.drylandTarget} onChange={(event) => setTargets({ ...targets, drylandTarget: event.target.value })} /></label><button>Spara från nästa måndag</button>{scheduled && <small>Senast planerat: {scheduled.strengthTarget} styrka + {scheduled.drylandTarget} land från {scheduled.startDate}</small>}</form>
  </section>
}
