...
function PlanningEditButton({ code, plan, date, group, onSaved, label = 'Redigera' }) {
  ...
  return <><button type="button" className="text-button" onClick={openEditor}>{label}</button>{open && form && <div className="planning-editor-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}><form className="planning-editor" onSubmit={save}><div className="planning-editor-head"><h2>{plan ? 'Redigera aktivitet' : 'Lägg till aktivitet'}</h2><button type="button" className="text-button" onClick={() => setOpen(false)}>Stäng</button></div><fieldset><legend>Vad planeras?</legend><div className="planning-type-buttons">{types.map(([value, text]) => <button type="button" className={form.activityType === value ? 'active' : ''} key={value} onClick={() => selectType(value)}>{text}</button>)}</div></fieldset>{form.activityType === 'swim' && <label>Huvudinriktning<select value={form.focus} onChange={(event) => setForm({ ...form, focus: event.target.value })}><option value="">Välj inriktning</option>{WORKOUT_FOCUSES.map(([value, text]) => <option value={value} key={value}>{text}</option>)}</select></label>}<label>Rubrik<input value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} /></label>{form.activityType === 'swim' && <label>Distans<select value={form.distanceMeters} onChange={(event) => setForm({ ...form, distanceMeters: event.target.value })}><option value="">Välj meter</option>{[2000, 3000, 4000, 5000, 6000, 7000].map((value) => <option value={value} key={value}>{value.toLocaleString('sv-SE')} m</option>)}</select></label>}<label>Tidsåtgång<select value={form.durationMinutes} onChange={(event) => setForm({ ...form, durationMinutes: event.target.value })}><option value="">Välj tid</option><option value="60">1 timme</option><option value="90">1,5 timmar</option><option value="120">2 timmar</option></select></label><label>Grupper<select value={form.targetGroups[0] || 'all'} onChange={(event) => setForm({ ...form, targetGroups: event.target.value === 'all' ? ['ungdom_orange', 'ungdom_svart', 'junior'] : [event.target.value] })}><option value="all">Alla grupper</option><option value="ungdom_orange">Ungdom Orange</option><option value="ungdom_svart">Ungdom Svart</option><option value="junior">Junior</option></select></label><label>Kommentar eller plats <input value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} /></label><button className="primary-button" type="submit">Spara planering</button></form></div>}</>
}
