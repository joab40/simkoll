...
function AnalysisDashboard({ code, profile, pointInfo, onBack, selfView = false }) {
  ...
  const createAiInsight = async (customInstructions = '') => {
    if (!data || aiLoading) return
    setAiLoading(true); setAiError('')
    try {
      const label = ANALYSIS_PERIODS.find((item) => item.key === period)?.label || 'vald period'
      const result = await apiRequest('/api/analytics', code, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'ai-insights', periodLabel: label, period, profileId: profile?.id, trainingContext: selfView ? null : trainingContext, customInstructions, data }) })
...
function AiInsightCard({ title = 'Veckans tränarsammanfattning', insight, createdAt, loading, error, onGenerate, swimmerView = false }) {
  ...
  return <section className={`ai-insight-card${expanded ? ' expanded' : ''}`}><div className="ai-insight-header"><div><p className="eyebrow">{swimmerView ? 'Din personliga analys' : 'AI-stöd för tränaren'}</p><h2>{title}</h2>{createdAt && <small>Senast skapad {new Date(createdAt).toLocaleString('sv-SE', { dateStyle: 'medium', timeStyle: 'short' })}</small>}</div><div className="ai-insight-actions">{insight && <button type="button" className="ai-expand-button" onClick={() => setExpanded((value) => !value)}>{expanded ? 'Minimera ↑' : 'Visa hela analysen ↓'}</button>}{onGenerate && <button className="secondary-button" onClick={() => onGenerate(customPrompt)} disabled={loading}>{loading ? 'Analyserar…' : insight ? 'Skapa ny analys' : 'Skapa analys'}</button>}</div></div>{onGenerate && <details className="ai-prompt-editor"><summary>🎛️ Anpassa analysens instruktion</summary><p>Beskriv vad tränaren vill att modellen fokuserar extra på. Grundreglerna för säker och försiktig analys ligger alltid kvar.</p><textarea maxLength={2000} value={customPrompt} onChange={(event) => setCustomPrompt(event.target.value)} placeholder="Exempel: Fokusera extra på om lägre mängd verkar sammanfalla med bättre fartkänsla inför tävlingen." /></details>}{error && <p className="form-error">{error}</p>}{insight ? <div className="ai-insight-body"><p>{insight.summary}</p>{insight.positives?.length > 0 && <div><strong>Det ser bra ut</strong>{insight.positives.map((item) => <span key={item}>✓ {item}</span>)}</div>}{insight.attention?.length > 0 && <div><strong>Följ upp</strong>{insight.attention.map((item) => <span key={item}>! {item}</span>)}</div>}{insight.limitations?.length > 0 && <small>Begränsningar: {insight.limitations.join(' · ')}</small>}</div> : <p className="ai-insight-empty">Ingen sparad analys för den valda perioden ännu.</p>}</section>
}
