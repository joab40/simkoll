...
function WebappSettings({ code }) {
  ...
  const save = async () => { await apiRequest('/api/goals', code, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'save-settings', settings }) }); setSaved(true); setTimeout(() => setSaved(false), 1800) }
  ...
  return <section className="webapp-settings"><div className="period-heading"><div><p className="eyebrow">Tränarverktyg</p><h1>Webapp-inställningar</h1><small>Styr vad som syns utan att radera någon data.</small></div></div><section className="settings-card"><h2>Simmarvyn</h2>{list('swimmer', swimmerFeatures)}</section><section className="settings-card"><h2>Tränarvyn</h2>{list('coach', coachFeatures)}</section><div className="settings-actions"><button className="primary-button" onClick={save}>Spara inställningar</button><button className="secondary-button" onClick={reset}>Återställ standard</button>{saved && <span className="settings-saved">Sparat ✓</span>}</div><p className="settings-note">Webapp-inställningar kan inte döljas och är alltid tillgängliga för tränare.</p></section>
}
