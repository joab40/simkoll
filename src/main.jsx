...
function Aljakten({ code, onBack, preview = false }) {
  ...
  const start = () => { const center = { x: 70, y: 240 }; const foodColors = ['#ffd166', '#f6bd60', '#f7aef8', '#8bd9e2', '#c9f05a']; const foods = Array.from({ length: 7 }, (_, index) => ({ x: 40 + Math.random() * 280, y: 70 + Math.random() * 355, radius: index % 3 === 0 ? 10 : index % 2 === 0 ? 8 : 6, points: index % 3 === 0 ? 2 : 1, color: foodColors[index % foodColors.length] })); const hazards = [{ x: -120, y: 135, speed: 30, size: 20, radius: 13, emoji: '🟡🐍', color: '#ffd166', phase: 0 }, { x: -300, y: 320, speed: 24, size: 27, radius: 19, emoji: '🟣🐍🐍', color: '#c5a7ff', phase: 2.5 }, { x: -520, y: 410, speed: 20, size: 23, radius: 17, emoji: '🔵🐍🐍🐍', color: '#8df5c1', phase: 4 }]; gameRef.current = { ...gameRef.current, running: true, heading: 0, snake: [center, { x: 56, y: 240 }, { x: 42, y: 240 }], hazards, target: { x: 260, y: 240 }, foods, score: 0, time: 0, last: performance.now(), frame: null }; setScore(0); setStatus('running'); window.setTimeout(() => gameRef.current.startLoop?.(), 0) }
