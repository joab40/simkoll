...
function CoachAppFeedback({ code }) {
  ...
  return <section className="app-feedback-page"><div className="period-heading"><div><p className="eyebrow">Tränarverktyg</p><h1>Hur kan vi göra Simkoll bättre?</h1><small>Samlad feedback från simmarna.</small></div><div className="big-count"><strong>{data?.total || 0}</strong><span>svar</span></div></div>{data?.total ? <><section className="feedback-summary-grid"><article className="coach-card"><p className="eyebrow">Helhetskänsla</p><strong className="feedback-average">{data.averageRating} <small>/ 5</small></strong></article><article className="coach-card"><p className="eyebrow">Vanligast uppskattat</p>{list(data.counts?.bestAreas).slice(0, 3).map(([key, count]) => <p className="feedback-stat" key={key}><span>{labels[key] || key}</span><b>{count}</b></p>)}</article><article className="coach-card"><p className="eyebrow">Vanligast att förbättra</p>{list(data.counts?.improveAreas).slice(0, 3).map(([key, count]) => <p className="feedback-stat" key={key}><span>{labels[key] || key}</span><b>{count}</b></p>)}</article></section><section className="coach-card feedback-comments"><p className="eyebrow">Fritext från simmarna</p><h2>Tankar och önskemål</h2>{data.comments?.length ? data.comments.map((item, index) => <blockquote key={`${item.createdAt}-${index}`}>“{item.comment}”<small>{new Date(item.createdAt).toLocaleDateString('sv-SE')}</small></blockquote>) : <p className="empty">Inga fritextsvar ännu.</p>}</section></> : <section className="coach-card empty-period"><span>💬</span><h2>Inga svar ännu</h2><p>När simmarna börjar lämna feedback visas sammanställningen här.</p></section>}</section>
}
