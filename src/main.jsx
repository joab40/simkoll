...
function Aljakten({ code, onBack, preview = false }) {
  ...
  useEffect(() => {
    ...
    const draw = () => { const game = gameRef.current; const now = game.time || 0; const gradient = ctx.createLinearGradient(0, 0, width, height); gradient.addColorStop(0, '#101d3e'); gradient.addColorStop(1, '#173f58'); ctx.fillStyle = gradient; ctx.fillRect(0, 0, width, height); ctx.fillStyle = 'rgba(115,225,226,.22)'; for (let i = 0; i < 30; i += 1) { const x = (i * 67 + now * (7 + i % 4)) % width; const y = height - ((now * (11 + i % 5) + i * 83) % (height + 40)); const radius = 1.5 + (i % 3); ctx.globalAlpha = .28 + ((Math.sin(now * 2 + i) + 1) / 2) * .5; ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill() } ctx.globalAlpha = 1; (game.otherEels || []).forEach((eel, index) => { const x = (eel.x + Math.sin(now * .55 + index) * 20 + width) % width; const y = eel.y + Math.sin(now * .8 + index * 2) * 8; ctx.globalAlpha = .3; ctx.fillStyle = index % 2 ? '#8bd9e2' : '#b0a8ee'; ctx.beginPath(); ctx.ellipse(x, y, eel.radius * 2, eel.radius * .48, Math.sin(now + index) * .12, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = 1 }); (game.foods || []).forEach((food, index) => { const pulse = Math.sin(now * 3.5 + index) * 1.4; ctx.fillStyle = food.color; ctx.shadowColor = food.color; ctx.shadowBlur = 10; ctx.beginPath(); ctx.arc(food.x, food.y, food.radius + pulse, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0; ctx.fillStyle = 'rgba(255,255,255,.7)'; ctx.beginPath(); ctx.arc(food.x - food.radius * .28, food.y - food.radius * .28, Math.max(1.5, food.radius * .18), 0, Math.PI * 2); ctx.fill() }); const snake = game.snake || []; const growth = .72 + Math.min(1.05, (game.score || 0) * .06); const bodyRadius = 6 + growth * 3.2; snake.forEach((part, index) => { ctx.fillStyle = index === 0 ? '#d9f56a' : `hsl(${156 + index * 2} 55% ${53 - Math.min(18, index)}%)`; ctx.beginPath(); ctx.arc(part.x, part.y, index === 0 ? bodyRadius + 1.5 : bodyRadius, 0, Math.PI * 2); ctx.fill(); if (index === 0) { ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(part.x - 4, part.y - 4, 3, 0, Math.PI * 2); ctx.arc(part.x + 4, part.y - 4, 3, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#173047'; ctx.beginPath(); ctx.arc(part.x - 4, part.y - 4, 1.3, 0, Math.PI * 2); ctx.arc(part.x + 4, part.y - 4, 1.3, 0, Math.PI * 2); ctx.fill() } }); ctx.fillStyle = 'rgba(255,255,255,.92)'; ctx.font = '800 24px Manrope, sans-serif'; ctx.textAlign = 'center'; ctx.fillText(String(game.score || 0), width / 2, 34) }
    const loop = (time) => { const game = gameRef.current; if (!game.running) { draw(); return } const delta = Math.min(.035, (time - game.last) / 1000); game.last = time; game.time = time / 1000; const dx = game.target.x - game.snake[0].x, dy = game.target.y - game.snake[0].y; const distance = Math.hypot(dx, dy) || 1; const speed = 96; const move = Math.min(distance, speed * delta); const head = { x: game.snake[0].x + dx / distance * move, y: game.snake[0].y + dy / distance * move }; const canHitSelf = game.snake.length > 24; const hitSelf = canHitSelf && game.snake.slice(20).some((part) => Math.hypot(head.x - part.x, head.y - part.y) < 9); const hitOtherEel = (game.otherEels || []).some((eel, index) => { const x = (eel.x + Math.sin(game.time * .55 + index) * 20 + width) % width; const y = eel.y + Math.sin(game.time * .8 + index * 2) * 8; return Math.hypot(head.x - x, head.y - y) < eel.radius + 9 }); if (head.x < 13 || head.x > width - 13 || head.y < 45 || head.y > height - 13 || hitSelf || hitOtherEel) { game.running = false; setStatus('over'); if (!preview) apiRequest('/api/points', code, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'submit-game-score', gameKey: 'aljakten', score: game.score }) }).then(setGameData).catch(() => {}); draw(); return } game.snake.unshift(head); while (game.snake.length > 9 + game.score * 2) game.snake.pop(); const eatenIndex = (game.foods || []).findIndex((food) => Math.hypot(head.x - food.x, head.y - food.y) < food.radius + 11); if (eatenIndex >= 0) { const eaten = game.foods[eatenIndex]; game.score += eaten.points; setScore(game.score); game.foods[eatenIndex] = { x: 22 + Math.random() * (width - 44), y: 60 + Math.random() * (height - 82), radius: eaten.radius, points: eaten.points, color: eaten.color } } draw(); game.frame = requestAnimationFrame(loop) }
    gameRef.current.startLoop = () => { gameRef.current.last = performance.now(); gameRef.current.frame = requestAnimationFrame(loop) }
    const moveTarget = (event) => { const rect = canvas.getBoundingClientRect(); gameRef.current.target = { x: (event.clientX - rect.left) * width / rect.width, y: (event.clientY - rect.top) * height / rect.height } }
    ...
    canvas.addEventListener('pointermove', moveTarget); window.addEventListener('keydown', keydown); draw(); return () => { canvas.removeEventListener('pointermove', moveTarget); window.removeEventListener('keydown', keydown); if (gameRef.current.frame) cancelAnimationFrame(gameRef.current.frame) }
  }, [code, preview])
  ...
  const start = () => { const center = { x: 70, y: 240 }; const foodColors = ['#ffd166', '#f6bd60', '#f7aef8', '#8bd9e2', '#c9f05a']; const foods = Array.from({ length: 7 }, (_, index) => ({ x: 40 + Math.random() * 280, y: 70 + Math.random() * 355, radius: index % 3 === 0 ? 10 : index % 2 === 0 ? 8 : 6, points: index % 3 === 0 ? 2 : 1, color: foodColors[index % foodColors.length] })); gameRef.current = { ...gameRef.current, running: true, snake: [center, { x: 56, y: 240 }, { x: 42, y: 240 }], otherEels: [{ x: 210, y: 120, radius: 8 }, { x: 300, y: 350, radius: 14 }], target: { x: 160, y: 240 }, foods, score: 0, time: 0, last: performance.now(), frame: null }; setScore(0); setStatus('running'); window.setTimeout(() => gameRef.current.startLoop?.(), 0) }
