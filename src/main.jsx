...
function WorkoutEditor({ code, responses }) {
  ...
  const rankedLibrary = library.map((workout) => { const answers = responses.filter((item) => item.type === 'after' && dateKey(responseDate(item)) === workout.date); const average = (key) => { const values = answers.map((item) => Number(item[key])).filter(Number.isFinite); return values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : null }; return { ...workout, pass: average('pass'), rpe: average('rpe') } }).sort((a, b) => { for (const key of criteria) { const value = (workout) => key === 'focus' ? (workout.focus ? 1 : 0) : key === 'distance' ? (workout.distanceMeters || 0) : key === 'duration' ? (workout.durationMinutes || 0) : (workout[key] ?? -1); const difference = value(b) - value(a); if (difference) return difference } return b.date.localeCompare(a.date) })
