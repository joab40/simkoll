...
function AuditLogs({ code }) {
  ...
  return <section className="audit-page"><div className="period-heading"><div><p className="eyebrow">Teknisk överblick</p><h1>Loggar</h1><small>Maskerade tekniska händelser och AI-användning. Råa IP-adresser sparas inte.</small></div><div className="big-count"><strong>{data.logs.length}</strong><span>senaste händelser</span></div></div><section className="audit-stats"><Stat title="AI-anrop" value={data.totals.calls} note={`${data.totals.successful} lyckade`} /><Stat title="Totala tokens" value={data.totals.totalTokens.toLocaleString('sv-SE')} note={`${data.totals.promptTokens.toLocaleString('sv-SE')} in · ${data.totals.completionTokens.toLocaleString('sv-SE')} ut`} /><Stat title="Inloggningar" value={data.logs.filter((item) => item.event_type.includes('login')).length} note="I senaste loggutdraget" /></section><section className="coach-card audit-ai"><h2>AI-anrop</h2>{data.aiUsage.length ? <div className="audit-list">{data.aiUsage.slice(0, 100).map((item) => <article key={item.id}><div><strong>{item.feature}</strong><small>{item.model || 'Okänd modell'} · {new Date(item.created_at).toLocaleString('sv-SE')}</small></div><span className={item.status === 'success' ? 'audit-ok' : 'audit-fail'}>{item.status === 'success' ? `${item.total_tokens || 0} tokens` : 'Misslyckat'}</span></article>)}</div> : <p className="empty">Inga AI-anrop loggade ännu.</p>}</section><section className="coach-card audit-events"><h2>Händelser</h2>{data.logs.length ? <div className="audit-list">{data.logs.map((item) => <article key={item.id}><div><strong>{eventLabels[item.event_type] || item.event_type}</strong><small>{item.role === 'coach' ? 'Tränare' : item.role === 'swimmer' ? 'Simmare' : item.role || 'Okänd roll'} · {new Date(item.created_at).toLocaleString('sv-SE')}</small></div><span className={item.status === 'success' ? 'audit-ok' : 'audit-fail'}>{item.status === 'success' ? 'OK' : 'Fel'}</span></article>)}</div> : <p className="empty">Inga händelser ännu.</p>}</section></section>
}
