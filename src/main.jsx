...
function Aljakten({ code, onBack, preview = false }) {
  ...
  useEffect(() => {
    ...
    const draw = () => { const game = gameRef.current; const gradient = ctx.createLinearGradient(0, 0, width, height); gradient.addColorStop(0, '#101d3e'); gradient.addColorStop(1, '#173f58'); ctx.fillStyle = gradient; ctx.fillRect(0, 0, width, height); ctx.fillStyle = 'rgba(115,225,226,.16)'; for (let i = 0; i < 18; i += 1) { const x = (i * 67 + (game.time || 0) * 9) % width; const y = 35 + ((i * 83) % (height - 60)); ctx.beginPath(); ctx.arc(x, y, 2 + (i % 3), 0, Math.PI * 2); ctx.fill() } const food = game.food; if (food) { ctx.fillStyle = '#ffd166'; ctx.shadowColor = '#ffd166'; ctx.shadowBlur = 12; ctx.beginPath(); ctx.arc(food.x, food.y, 7, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0 } const snake = game.snake || []; snake.forEach((part, index) => { ctx.fillStyle = index === 0 ? '#c9f05a' : `hsl(${156 + index * 2} 55% ${53 - Math.min(18, index)}%)`; ctx.beginPath(); ctx.arc(part.x, part.y, Math.max(7, 13 - index * .18), 0, Math.PI * 2); ctx.fill() }); ctx.fillStyle = 'rgba(255,255,255,.92)'; ctx.font = '800 24px Manrope, sans-serif'; ctx.textAlign = 'center'; ctx.fillText(String(game.score || 0), width / 2, 34) }
  ...
  const start = () => { const center = { x: 70, y: 240 }; gameRef.current = { ...gameRef.current, running: true, snake: [center, { x: 55, y: 240 }, { x: 40, y: 240 }], target: { x: 160, y: 240 }, food: { x: 245, y: 220 }, score: 0, time: 0, last: performance.now(), frame: null }; setScore(0); setStatus('running'); window.setTimeout(() => gameRef.current.startLoop?.(), 0) }
