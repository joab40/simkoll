  ...
  return (
    <section className="swimmers-section">
      ...
      {visibleProfiles.length ? <div className="swimmer-grid">{visibleProfiles.map((profile) => {
        ...
        const trainingGoals = [{ icon: '🏊', label: 'Simning', completed: profileSessions.filter((item) => item.type === 'swim').length, target: swimGoal?.target }, { icon: '🤸', label: 'Landträning', completed: profileSessions.filter((item) => item.type === 'dryland').length, target: crossGoal?.drylandTarget }, { icon: '🏋️', label: 'Styrka', completed: profileSessions.filter((item) => item.type === 'strength').length, target: crossGoal?.strengthTarget }].filter((item) => item.target > 0)
