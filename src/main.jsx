...
function WeeklySwimCard({ training, onOpen, onToggle, onPlan }) {
  ...
  const toggle = async (date, slot, checked) => { const key = `${date}-${slot}`; const previous = localSessions || []; const type = slot.includes('swim') ? 'swim' : slot; const next = checked ? [...previous.filter((item) => !(item.date === date && item.slot === slot)), { date, slot, type }] : previous.filter((item) => !(item.date === date && item.slot === slot)); setLocalSessions(next); setSaving(key); try { const result = await onToggle(date, slot, checked); if (result?.message) setCheer(result.message) } catch (error) { setLocalSessions(previous); window.alert(error.message) } finally { setSaving('') } }
  const togglePlan = async (date, slot, checked) => { const key = `plan-${date}-${slot}`; const previous = localPlans || []; const next = checked ? [...previous.filter((item) => !(item.date === date && item.slot === slot)), { date, slot, weekStart: dateKey(start) }] : previous.filter((item) => !(item.date === date && item.slot === slot)); setLocalPlans(next); setSaving(key); try { const result = await onPlan(date, slot, checked); if (result?.message) setCheer(result.message) } catch (error) { setLocalPlans(previous); window.alert(error.message) } finally { setSaving('') } }
