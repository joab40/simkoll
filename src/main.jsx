...
function Swimmers({ profiles, pendingProfiles, onProfilesChange, responses, code }) {
  ...
  const visibleProfiles = profiles
    ...
    .sort((a, b) => {
      ...
      const activityRank = (item) => item?.type === 'after' ? 3 : item?.type === 'before' && item.speedFeeling != null && item.raceConcern != null ? 2 : item ? 1 : 0
  ...
  return (
    <section className="swimmers-section">
      ...
      {visibleProfiles.length ? <div className="swimmer-grid">{visibleProfiles.map((profile) => {
        ...
        const raceCheckin = todayItem?.type === 'before' && todayItem.speedFeeling != null && todayItem.raceConcern != null
        const status = hasSwimToday ? ['after', '✓ Har tränat idag'] : todayItem && ({ sick: ['sick', '🤒 Sjuk idag'], rest: ['rest', '⏸️ Tränar inte idag'], before: [raceCheckin ? 'race' : 'before', raceCheckin ? '🏁 Ska tävla idag' : '→ Ska träna idag'], after: ['after', '✓ Har tränat idag'] }[todayItem.type])
        const attention = hasSwimToday ? '🏊 Tränat idag' : raceCheckin ? '🏁 Ska tävla idag' : todayItem?.type === 'after' ? '🏊 Tränat idag' : todayItem?.type === 'sick' ? '🤒 Sjuk idag' : todayItem?.type === 'rest' ? '⏸️ Tränar inte idag' : todayItem?.body <= 2 ? `⚠️ Kroppen ${todayItem.body}/5` : todayItem?.feeling <= 2 ? `⚠️ Känsla ${todayItem.feeling}/5` : todayItem ? '✓ Aktiv idag' : 'Ingen aktivitet idag'
